    ...
    <div className="flex items-center gap-1.5 mt-2">
      {[
        ...
        <span key={label} className="flex items-baseline gap-0.5">
          {i > 0 && (
            <span className="tabnum" style={{ color: 'var(--kinpaku-deep)', marginRight: 2 }}>
              :
            </span>
          )}
          <span
          ...
          <span className="eyebrow" style={{ fontSize: '0.5rem', color: 'var(--text-disabled)' }}>
            {label}
          </span>
        </span>
      ))}
...
export default function FavoriteTeamCard({ team, matches, timeZone, onRemove }: Props) {
  ...
  function getScore(match: Match) {
    ...
    if (match.homeTeam.id === team.id) return `${homeScore} – ${awayScore}`
    return `${awayScore} – ${homeScore}`
  }

  return (
    <div
      className="glass-card relative overflow-hidden mb-5"
      style={{ borderRadius: 'var(--r-lg)', borderLeft: '3px solid var(--kinpaku)' }}
    >
      {/* Gold seam top */}
      <div
        className="absolute top-0 left-0 right-0 h-px"
        style={{ background: 'linear-gradient(to right, var(--kinpaku), transparent)' }}
        aria-hidden="true"
      />
      ...
      <div className="p-4">
        ...
        <div className="flex items-center justify-between mb-3">
          <span
            ...
            style={{ color: 'var(--kinpaku)', letterSpacing: '0.12em' }}
          >
          ...
          <button
            onClick={onRemove}
            aria-label="Quitar equipo favorito"
            className="flex items-center gap-1 transition-colors"
            style={{ color: 'var(--text-disabled)' }}
            onMouseEnter={(e) =>
              ((e.currentTarget as HTMLButtonElement).style.color = 'var(--vermilion)')
            }
            onMouseLeave={(e) =>
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <p
              ...
              style={{ color: 'var(--champagne)', fontFamily: 'var(--font-hanken)', fontSize: '1.1rem' }}
            >
              {team.shortName ?? team.name}
            </p>
            <p className="eyebrow mt-0.5" style={{ color: 'var(--text-disabled)', fontSize: '0.58rem' }}>
              {team.tla}
            </p>
          </div>
        </div>
        ...
        {displayMatch ? (
            ...
            <div className="flex items-center gap-2.5">
              {getOpponent(displayMatch).crest ? (
                <div className="relative w-8 h-8 flex-shrink-0">
                  <Image
                    src={getOpponent(displayMatch).crest}
                    alt={getOpponent(displayMatch).name}
                    fill
                    unoptimized
                    className="object-contain"
                    sizes="32px"
                ...
                <div
                  ...
                  style={{ background: 'var(--graphite)', borderRadius: 'var(--r-sm)' }}
                >
                  <span className="eyebrow text-[8px]" style={{ color: 'var(--text-faint)' }}>
                    {getOpponent(displayMatch).tla}
                  </span>
                </div>
              )}
              ...
              <div className="flex-1 min-w-0">
                ...
                <p
                  ...
                  style={{ fontSize: '0.6rem', color: 'var(--text-disabled)' }}
          ...
          <div
            className="rounded-lg px-3 py-2.5"
            style={{ background: 'var(--raised-lacquer)', border: '1px solid var(--glass-border)' }}
          >
