...
interface SportsDBPlayer { strThumb?: string; strCutout?: string; strHeight?: string; strWeight?: string; strBirthLocation?: string }

async function getWikiSummary(slug: string): Promise<WikiSummary | null> {
  try {
    const res = await fetch(
      `https://en.wikipedia.org/api/rest_v1/page/summary/${encodeURIComponent(slug)}`,
      { headers: { 'Api-User-Agent': 'Mundial2026Promo/0.1' }, next: { revalidate: 86400 } },
    )
...
export default async function WikiPlayerPage({
  ...
  const [summary, sportsdb] = await Promise.all([getWikiSummary(decodedSlug), getSportsDBPlayer(displayName)])
  ...
  const posLabel    = pos ? (t(pos as 'Goalkeeper' | 'Defence' | 'Midfield' | 'Offence') ?? pos) : null
  const dobFormatted = dob
    ? new Intl.DateTimeFormat(locale, { day: 'numeric', month: 'long', year: 'numeric' }).format(new Date(dob))
    : null
  const initials    = name.split(' ').map((w: string) => w[0] ?? '').join('').slice(0, 2).toUpperCase()
  const photoSource = photo ? (sportsdb?.strThumb || sportsdb?.strCutout ? 'TheSportsDB' : 'Wikipedia') : null

  return (
    <PlayerDetail
      name={name} initials={initials} photoUrl={photo}
      posLabel={posLabel} posColor={posColor} shirtNumber={null}
      nationality={nationality} flag={flag} age={age} dobFormatted={dobFormatted}
      caps={caps} goals={goals} club={club}
      height={sportsdb?.strHeight ?? null} weight={sportsdb?.strWeight ?? null}
      birthLocation={sportsdb?.strBirthLocation ?? null}
      bio={summary?.extract ?? null} photoSource={photoSource} backHref={backHref}
    />
  )
}
